...
type Client struct {
	...
	disableIrcRelay bool
}

/*
NewClient creates a new Telegram bot client
*/
func NewClient(settings *internal.TelegramSettings, ircsettings *internal.IRCSettings, imgur *internal.ImgurSettings, tgapi *tgbotapi.BotAPI, logger internal.DebugLogger, disableIrcRelay bool) *Client {
	logger.LogInfo("Creating new Telegram bot client...")
	return &Client{api: tgapi, Settings: settings, IRCSettings: ircsettings, ImgurSettings: imgur, logger: logger, disableIrcRelay: disableIrcRelay}
}
...
func (tg *Client) StartBot(errChan chan<- error, sendMessage func(string)) {
	...
	if !tg.disableIrcRelay {
		...
		tg.logger.LogInfo("Telegram -> IRC relay disabled, but Telegram bot remains active for IRC -> Telegram messages")
		// Block forever to keep the goroutine alive
		select {}
	}
}
